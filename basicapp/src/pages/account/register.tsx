...
export default function Register() {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (result.statusCode === 200) {
      ...
      showToast(
        result.responseData.message ?? "Error al crear la cuenta",
        "error"
      );
    ...
    <AnimatedContainer>
      <div className="flex w-full max-w-md space-y-8 place-self-center">
        <form
          ...
          className="flex items-center justify-center my-10 px-4 md:min-w-1/4 w-full py-12 sm:px-6 lg:px-8 bg-zinc-100 dark:bg-zinc-800 shadow-2xl"
        >
          <div className="w-full max-w-md space-y-8">
            ...
            <div className="flex flex-col space-y-4 pt-1">
              <button
                type="submit"
                disabled={isLoading}
                className="w-full flex flex-row h-12 justify-center shadow-lg shadow-red-600/10 items-center gap-x-2 bg-red-600 hover:bg-red-500 hover:scale-105 active:scale-90 text-white font-medium disabled:bg-red-300 transition-all"
              >
                {isLoading && (
